...
public class AuthenticationMessageHandler : DelegatingHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation("Received 401 for {Path}, attempting to refresh token...", path);
    ...
    private async Task AttachTokenAsync(HttpRequestMessage request)
    {
        ...
        {
            _logger.LogError(ex, "Error attaching token to request");
        }
    }
    ...
    private async Task<bool> TryRefreshTokenAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Calling refresh-token endpoint...");
            ...
            if (refreshResponse.IsSuccessStatusCode)
            {
                ...
                if (result?.Success == true && result.Data != null)
                {
                    ...
                    if (_authenticationStateProvider is JwtAuthenticationStateProvider jwtProvider)
                    {
                        ...
                        _logger.LogInformation("Token refreshed and auth state updated. New token expires in {Seconds}s", result.Data.ExpiresInSeconds);
                        return true;
                    }
                }
                else
                {
                    _logger.LogWarning("Refresh token response was not successful: {Message}", result?.Message);
                }
            }
            else
            {
                _logger.LogWarning("Refresh token request failed with status: {StatusCode}", refreshResponse.StatusCode);
...
/// <summary>
/// DTO for refresh token response - used internally by the handler
/// </summary>
internal class TokenRefreshResultDto
{
